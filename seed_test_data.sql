-- ====================================================================
-- Peeple API Test Data Seeding Script (with Premium Features)
-- ====================================================================
...
TRUNCATE TABLE user_subscriptions CASCADE; -- Added
TRUNCATE TABLE user_consumables CASCADE;   -- Added
...
SELECT setval(pg_get_serial_sequence('user_subscriptions', 'id'), COALESCE((SELECT MAX(id) FROM user_subscriptions), 0) + 1, false); -- Added
-- user_consumables has a composite PK, no sequence needed

-- Insert Test Users (Unchanged from your version)
...
-- Insert Filters (Unchanged from your version)
...
-- Insert App Open Logs (Unchanged from your version)
...
-- Insert Likes FROM Alice (ID 1) - UPDATED
INSERT INTO likes (liker_user_id, liked_user_id, interaction_type) VALUES
    (1, 8, 'standard');  -- Alice liked Hank (Standard)

-- Insert Dislikes involving Alice (ID 1) - Unchanged
INSERT INTO dislikes (disliker_user_id, disliked_user_id) VALUES
    (1, 9),   -- Alice disliked Ivy
    (10, 1);  -- Jack disliked Alice

-- Setup Premium Features -- ADDED
-- Give Bob (ID 2) unlimited likes for the next 7 days
INSERT INTO user_subscriptions (user_id, feature_type, expires_at) VALUES
    (2, 'unlimited_likes', NOW() + INTERVAL '7 days');

-- Give Charlie (ID 3) 5 Roses
...
-- SELECT u.id, u.name FROM users u WHERE u.id BETWEEN 1 AND 20 ORDER BY u.id;
-- SELECT * FROM likes;
